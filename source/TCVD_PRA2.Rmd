...
#númericas
#Shapiro-Wilk
print(shapiro.test(data$oldpeak))
print(leveneTest(oldpeak ~ output, data = data,))
print(shapiro.test(data$thalachh))
print(leveneTest(thalachh ~ output, data = data,))


#categoricas
# Variable "caa"
table_caa <- table(data$caa, data$output)
print(chisq.test(table_caa))

# Variable "exng"
table_exng <- table(data$exng, data$output)
print(chisq.test(table_exng))

# Variable "cp"
table_cp <- table(data$cp, data$output)
...
Respecto a oldpeak, podemos observer como no sigue una distribución normal, ni tienen igualdad de varianzas.

Lo mismo se aplica a oldpeak, podemos observer como no sigue una distribución normal, ni tienen igualdad de varianzas.

respecto

Veamos cómo interpretar cada una de las pruebas que realizamos:

### Normalidad --> Shapiro-Wilk

1. Variable `oldpeak`: El valor p es 2.615e-16, que es menor que 0.05. Por lo tanto, rechazamos la hipótesis nula y concluimos que los datos de `oldpeak` no están normalmente distribuidos.

2. Variable `thalachh`: El valor p es 9.187e-05, que es menor que 0.05. Por lo tanto, rechazamos la hipótesis nula y concluimos que los datos de `thalachh` no están normalmente distribuidos.

### Homogeneidad de la varianza --> Levene

1. Variable `oldpeak`: El valor p es 2.232e-08, que es menor que 0.05. Por lo tanto, rechazamos la hipótesis nula y concluimos que las varianzas de los grupos para `oldpeak` no son homogéneas.

2. Variable `thalachh`: El valor p es 0.02553, que es menor que 0.05. Por lo tanto, rechazamos la hipótesis nula y concluimos que las varianzas de los grupos para `thalachh` no son homogéneas.
...
```{r}
# Crear las variables dummy para 'cp', 'caa', 'thall'
dummy_cp <- model.matrix(~ cp - 1, data = data)
dummy_caa <- model.matrix(~ caa - 1, data = data)
dummy_exng <- model.matrix(~ exng - 1, data = data)

# Convertir las matrices a data frames
dummy_cp_df <- as.data.frame(dummy_cp)
dummy_caa_df <- as.data.frame(dummy_caa)
dummy_exng_df <- as.data.frame(dummy_exng)

# Añadir prefijo a las columnas para identificar a qué variable pertenecen
colnames(dummy_cp_df) <- paste("cp", colnames(dummy_cp_df), sep = "_")
colnames(dummy_caa_df) <- paste("caa", colnames(dummy_caa_df), sep = "_")
colnames(dummy_exng_df) <- paste("exng", colnames(dummy_exng_df), sep = "_")

# Unir los data frames de las variables dummy con el data frame original
data2 <- cbind(data, dummy_cp_df, dummy_caa_df, dummy_exng_df)

# Eliminar las columnas originales 'cp', 'caa', 'exng'
data2$cp <- NULL
data2$caa <- NULL
data2$exng <- NULL
